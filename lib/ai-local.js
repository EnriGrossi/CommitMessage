...
Your JSON response:`;
    ...
    try {
        ...
        // Fallback: build a message from inferred data if model output is garbage
        const fallbackType = inferredType || 'chore';
        const fallbackScope = inferredScope ? `(${inferredScope})` : '';
        return `${fallbackType}${fallbackScope}: update ${fileNames[0] || 'files'}`;
    } catch (e) {
...
Your JSON response:`;
    ...
    try {
        ...
        const sanitized = sanitizeCommitMessage(raw);
        if (sanitized) return sanitized;
        return originalMessage; // If refinement fails, keep original
    } catch (e) {
        console.warn("Failed to parse grammar-enforced JSON", e);
    }

    const sanitized = sanitizeCommitMessage(cleanedResponse);
    return sanitized || originalMessage;
}
