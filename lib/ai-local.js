...
// Optimal CPU thread count for llama.cpp: physical cores to avoid SMT cache thrashing
function getOptimalThreadCount() {
    const total = os.cpus().length;
    if (total <= 2) return total;
    if (total <= 4) return total;
    return Math.min(8, Math.floor(total / 2));
}
...
function analyzeDiffContent(diff) {
    ...
    const flushFile = () => {
        if (currentFile) {
            ...
            if (isNewFile) analysis.newFiles.push(currentFile);
            if (isDeletedFile) analysis.deletedFiles.push(currentFile);
    ...
    for (const f of analysis.files) {
        const parts = [];
        if (f.isNew) parts.push('NEW');
        if (f.isDeleted) parts.push('DELETED');
...
function inferCommitType(analysis, fileNames) {
    ...
    // New files are typically a feature or chore, not a refactor
    const allNew = analysis.newFiles && analysis.newFiles.length === fileNames.length;
    const allDeleted = analysis.deletedFiles && analysis.deletedFiles.length === fileNames.length;
    ...
    const configFiles = fileNames.filter(f =>
        ...
        f.includes('.gitlab-ci') || f.includes('.github/')
    );
    if (configFiles.length === fileNames.length) {
        if (allNew) return 'ci';
        return 'chore';
    }

    if (allNew) return 'feat';
...
export async function generateCommitMessage(modelPath, diff, onProgress, { regenerate = false } = {}) {
    ...
    if (analysis.newFiles.length > 0) {
        hints.push(`NEW files added: ${analysis.newFiles.join(', ')}`);
    }
    if (analysis.deletedFiles.length > 0) {
        hints.push(`DELETED files: ${analysis.deletedFiles.join(', ')}`);
    }
    hints.push(`Files changed: ${analysis.summary.join(', ')}`);
    hints.push(`Total: +${analysis.additions} additions, -${analysis.deletions} deletions`);

    // When all files are new, the commit message should focus on adding the file(s), not their content
    const newFileHint = analysis.newFiles.length > 0 && analysis.newFiles.length === fileNames.length
        ? '\nIMPORTANT: All files in this diff are NEW. The commit message should describe ADDING these files, not their internal content details.\n'
        : '';

    // For small diffs, include the raw diff. For larger ones, avoid repeating the digest
    const diffForPrompt = processedDiff.length <= 3000 ? processedDiff : '(summarized in KEY CHANGES above)';

    // On regeneration, add variation to avoid identical outputs
    const variationHint = regenerate
        ? `\nIMPORTANT: Generate a DIFFERENT commit message than before. Try a different type, scope, or focus on a different aspect of the changes.\n`
        : '';
...
- For NEW files, describe what was added (e.g., "add gitlab ci pipeline")
- For DELETED files, describe what was removed
...
- ci(gitlab): add ci/cd pipeline config
...
Your JSON response:`;
    ...
    // Release context memory so subsequent calls can allocate a new one
...
Diff (for reference):
${processedDiff.length <= 3000 ? processedDiff : '(summarized in KEY CHANGES above)'}
